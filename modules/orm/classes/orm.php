...
class ORM {
	...
	public function __get($column)
	{
		...
		{
			if( $this->unique_key($this->object[$this->primary_key]) !== $this->primary_key)
			{
	...
	public function with($target_path)
	{
		...
		else
		{
			...
			$join_col1 = $parent->foreign_key($target_name, $join_table);
		}

		// Join the related object into the result
		$this->db_builder->join($this->db->table_prefix().$join_table, 'LEFT')->on($join_col1, "=", $join_col2);
	 ...
	 * @return  boolean
	 */
	public function validate(Validate $array, $save = FALSE)
	{
	...
	public function save()
	{
		if ( ! empty($this->changed))
		{
			...
			if ( ! $this->empty_primary_key() AND ! isset($this->changed[$this->primary_key]))
			{
				...
				$query = DB::update($this->db->table_prefix().$this->table_name)
					->set($data)
			...
			else
			{
				...
				$result = DB::insert($this->db->table_prefix().$this->table_name)
			...
			foreach ($this->changed_relations as $column => $values)
			{
				...
				if ( ! empty($added))
				{
					foreach ($added as $id)
					{
						...
						DB::insert($this->db->table_prefix().$join_table)
				...
				if ( ! empty($removed))
				{
					DB::delete($this->db->table_prefix().$join_table)
	...
	public function delete($id = NULL)
	{
		...
		// Delete this object
		DB::delete($this->db->table_prefix().$this->table_name)
	...
	public function count_all()
	{
		...
		return $this->db->count_records($this->db->table_prefix().$this->table_name);
	}
	...
	{
		// Proxy to database
		return $this->db->list_columns($this->db->table_prefix().$this->table_name);
	}
...
/*	public function field_data()
	...
	public function foreign_key($table = NULL, $prefix_table = NULL)
	{
		...
		{
			if ( ! is_string($table) OR ! array_key_exists($table.$this->foreign_key_suffix, $this->object))
			{
				// Use this table
	...
	{
		$this->db_builder->from($this->db->table_prefix().$this->table_name);
...
//			$this->db_builder->order_by($sorting);
	...
	protected function load_relations(array $target, ORM $model)
	{
		$result = DB::select()->select(array($model->foreign_key(NULL), 'id'))
			->from($this->db->table_prefix().$target['table_name'])
	...
	/**
	 * Executes find() if PK set, but model was not loaded
	 * @chainable
	 * @return    ORM
	 */
	protected function load() {
		...
		{
			// load only columns that haven't been modified
